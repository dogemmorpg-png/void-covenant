    ...
    <div className="h-full w-full overflow-y-auto select-none px-2.5 sm:px-4 pt-2 pb-28 custom-scrollbar">
      <div className="max-w-md mx-auto w-full flex flex-col gap-3">
        ...
        <div className={`rounded-3xl p-4 sm:p-5 w-full relative overflow-hidden flex flex-col gap-3.5 border transition-all duration-300 ${
          ...
          <div className="text-center border-b border-gray-800/80 pb-3 flex flex-col items-center relative z-10">
            <h2 className="font-display font-black text-xl sm:text-2xl text-white tracking-widest text-shadow-gold">
              THE ENDLESS ABYSS
            </h2>
            <p className="text-[11px] text-gray-400 mt-1 max-w-xl mx-auto font-sans leading-relaxed">
            ...
            <div 
              ...
              className="mt-2.5 inline-flex items-center gap-2.5 bg-gradient-to-r from-[#061c12] via-black to-[#061c12] border border-emerald-500/50 hover:border-emerald-400 rounded-full py-1 pl-3 pr-1.5 shadow-[0_0_18px_rgba(16,185,129,0.25)] select-none transition-all cursor-pointer group active:scale-95"
              title="Click to Refill Energy with Dark Shards"
            >
              <img 
                src="/icons/icon_energy.webp" 
                alt="Energy" 
                className="w-4 h-4 sm:w-5 sm:h-5 object-contain drop-shadow-[0_0_8px_rgba(16,185,129,0.9)] group-hover:scale-110 transition-transform" 
              />
              ...
              <span className="font-mono text-[10px] sm:text-[11px] text-emerald-300/90 font-bold border-l border-emerald-500/30 pl-2 pr-1">
                {timeUntilRegen ? `+1 in ${timeUntilRegen}` : 'Full Energy'}
              </span>
              <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-gradient-to-b from-emerald-500 to-emerald-700 group-hover:from-emerald-400 group-hover:to-emerald-600 text-black flex items-center justify-center border border-emerald-300/50 shadow-[0_0_10px_rgba(16,185,129,0.6)] shrink-0">
          ...
          <div className="flex items-center justify-between bg-black/40 border border-gray-800/50 rounded-[28px] py-3 px-4 sm:px-6 shadow-inner relative z-10">
            <button 
              onClick={handlePrev}
              disabled={viewingFloor === 1}
              className="p-3 rounded-2xl bg-black/40 border border-[#ebd09b]/25 hover:border-[#ebd09b]/60 text-[#ebd09b] hover:bg-[#ebd09b]/10 disabled:opacity-20 disabled:pointer-events-none transition-all cursor-pointer active:scale-90 shrink-0 shadow-md"
            ...
            <div className="flex flex-col items-center select-none text-center px-2">
              ...
              <div className="text-5xl sm:text-6xl font-display font-black text-white text-shadow-gold leading-none my-1">
                {viewingFloor}
              </div>
          ...
          <div className="flex flex-col gap-3 relative z-10">
            ...
            <div className="space-y-1.5">
              ...
              <div className="bg-black/50 border border-gray-800/60 rounded-2xl p-3.5 sm:p-4 flex items-center gap-3.5 shadow-inner">
                <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center border shrink-0 ${
                  ...
                  {selectedStage.enemyHeroImage?.startsWith('/') ? (
                    ...
                    <Crown className="w-8 h-8 text-purple-500 drop-shadow-[0_0_12px_rgba(168,85,247,0.8)] animate-pulse" />
                  ) : (
                    <Skull className="w-7 h-7 text-red-500/90" />
                  )}
                </div>
                <div>
                  <h4 className="font-display font-bold text-white text-base sm:text-lg leading-none">
            ...
            <div className="space-y-1.5">
              ...
              <div className="bg-black/50 border border-gray-800/60 rounded-2xl p-2.5 sm:p-3.5 flex items-center justify-around gap-2 shadow-inner">
                {/* Gold Pill */}
                ...
                {selectedStage.shardsReward > 0 ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-2 rounded-xl bg-white/[0.03] border border-emerald-500/20 hover:border-emerald-400/50 hover:bg-emerald-500/[0.06] transition-all cursor-default group">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-mono font-black text-emerald-400 text-sm sm:text-base leading-none">+{selectedStage.id * 16 + 32}</span>
                    </div>
          ...
          {selectedStage.cardReward && (
            <div className="bg-black/50 border border-emerald-500/20 p-3 rounded-2xl flex items-center justify-between shadow-inner relative z-10">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-full bg-emerald-950/40 flex items-center justify-center border border-emerald-500/30 shrink-0">
                  <Award className="w-4 h-4 text-emerald-400" />
                </div>
                <div>
                  <span className="text-emerald-400 font-bold text-xs block leading-tight">Guaranteed Card Drop</span>
                  <span className="text-[10px] text-gray-400 font-mono mt-0.5">{selectedStage.cardReward.name}</span>
                </div>
              </div>
              <span className="text-[9px] bg-emerald-950/40 text-emerald-400 px-2.5 py-0.5 rounded-lg font-mono border border-emerald-500/20">
                Guaranteed
              </span>
            </div>
          )}

          {/* 4. Action Buttons: BATTLE & SWEEP matching PC dark slate style */}
          <div className="pt-2 border-t border-gray-800/80 flex flex-col gap-2 relative z-10">
            <button
              onClick={handleStart}
              className={`w-full font-display font-bold tracking-widest py-3 sm:py-3.5 px-6 rounded-xl transition-all duration-300 flex items-center justify-center gap-4 cursor-pointer hover:scale-[1.02] active:scale-[0.98] text-sm uppercase ${
                isBoss 
            ...
            >
              ...
              <span className={`flex items-center gap-1.5 bg-black/50 border rounded-full px-3 py-0.5 font-mono text-base sm:text-lg font-bold text-emerald-400 shadow-inner ${
                isBoss ? 'border-red-500/30' : 'border-[#ebd09b]/35'
              }`}>
                {selectedStage.energyCost}
                <img src="/icons/icon_energy.webp" alt="Energy" className="w-5 h-5 object-contain brightness-110 drop-shadow-[0_0_6px_rgba(16,185,129,0.45)]" />
              </span>
            </button>
            ...
            {canSweep ? (
              <button
                ...
                className="w-full font-display font-bold tracking-widest py-3 sm:py-3.5 px-6 rounded-xl transition-all duration-300 flex items-center justify-center gap-4 cursor-pointer hover:scale-[1.02] active:scale-[0.98] text-sm uppercase bg-gradient-to-b from-[#1b122c] via-[#0e071a] to-[#06020c] border-2 border-purple-600/50 hover:border-purple-400 text-purple-400 hover:text-white shadow-[0_0_15px_rgba(147,51,234,0.2)] hover:shadow-[0_0_25px_rgba(168,85,247,0.35)] disabled:opacity-40"
              >
                <FastForward className="w-5 h-5" /> 
                <span className="mr-0.5">SWEEP</span>
                <span className="flex items-center gap-1.5 bg-black/50 border border-purple-500/30 rounded-full px-3 py-0.5 font-mono text-base sm:text-lg font-bold text-emerald-400 shadow-inner">
                  {selectedStage.energyCost}
                  <img src="/icons/icon_energy.webp" alt="Energy" className="w-5 h-5 object-contain brightness-110 drop-shadow-[0_0_6px_rgba(16,185,129,0.45)]" />
                </span>
              </button>
              ...
              <div className="text-center py-0.5 text-[10px] font-mono text-gray-500">
                {stageStars === 0 ? 'Clear floor to earn stars' : `${stageStars}/3 Stars • 3★ unlocks Sweep`}
              </div>
            )}
          </div>
