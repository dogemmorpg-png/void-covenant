...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 1. Fetch current player's profile
    ...
    if (updateError) {
      console.error('Failed to update player profile during matchmaking cancel:', updateError);
    }

    return res.status(200).json({ success: true, profile: profileData });

  } catch (error: any) {
    console.error('Matchmaking cancel API error:', error);
