...
// api/sync.ts
var JWT_SECRET = process.env.JWT_SECRET || "fallback-secret-for-dev-only-change-in-prod";
function calculateEnergy(profile) {
  ...
  const pveRegenInterval = 20 * 60 * 1e3;
  const pvpRegenInterval = 15 * 60 * 1e3;
  const timePassedPve = Math.max(0, now - lastPve);
  const timePassedPvp = Math.max(0, now - lastPvp);
  let currentPve = profile.pveEnergy !== void 0 ? profile.pveEnergy : pveMax;
  let currentPvp = profile.pvpEnergy !== void 0 ? profile.pvpEnergy : pvpMax;
...
async function handler(req, res) {
  ...
  try {
    ...
    const { data: profileRows, error: fetchError } = await supabase.from("profiles").select("data").eq("wallet_address", walletAddress).limit(1);
