...
async function handler(req, res) {
  ...
  try {
    ...
    const card1 = profile.collection.find((c) => c.id === cardId1);
    const card2 = profile.collection.find((c) => c.id === cardId2);
    if (!card1 || !card2) {
    ...
    profile.collection = profile.collection.filter((c) => c.id !== card1.id && c.id !== card2.id);
