...
function generateRandomCards(packType, numCards) {
...
function generateRandomEquipment(packType, numEquips) {
  let selectedEquips = [];
...
async function handler(req, res) {
  ...
  try {
    ...
    const { data: profileData, error: profileError } = await supabase.from("profiles").select("data").eq("wallet_address", walletAddress).single();
