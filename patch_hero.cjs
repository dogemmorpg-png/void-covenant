...
// 3. Inject tab switcher and logic in render
const tabsRender = `      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        
        {/* Left Stats Panel */}
        <div className="md:col-span-5 bg-[#151a21] border border-[#c5a880]/30 rounded-3xl p-6 flex flex-col items-center h-fit">
...
content = content.replace(
  /<div className="grid grid-cols-1 md:grid-cols-12 gap-8">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*\{\/\* Inventory Panel \*\/\}\s*\{renderInventoryList\(\)\}\s*<\/div>\s*\);\s*\};\s*$/m,
  tabsRender
);

fs.writeFileSync('src/components/HeroInventoryView.tsx', content);
console.log('HeroInventory patched successfully');
